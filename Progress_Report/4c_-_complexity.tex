...
Complexity was analysed using the standard \textbf{operation-counting} approach~\citep{cormen2009}: (1) identify all operations (heap insertions, extractions, edge relaxations), (2) count the maximum number of each as a function of $|V|$ and $|E|$, (3) multiply by per-operation cost, and (4) verify empirically.
...
\begin{enumerate}
    ...
    \item \textbf{Expected case ($\tau>0$):} Let $|E'|\leq|E|$ denote edges surviving the threshold. Pruned edges are checked in $O(1)$ without heap insertion. The expected complexity is:
    \[
...
\midrule
1,000  & 0.64 & 0.058 & 11.0$\times$ & 30.6$\times$ \\
5,000  & 3.60 & 0.064 & 55.8$\times$ & 104.9$\times$ \\
10,000 & 7.25 & 0.064 & 89.1$\times$ & 173.9$\times$ \\
\bottomrule
\end{tabular}
\caption[Execution time scaling by graph size]{Median execution time at $\tau=0.01$. Baseline time scales roughly linearly with $|V|$ (consistent with $O(|V|\log|V|)$ for sparse graphs). Pruned time remains nearly constant because the threshold eliminates most of the graph.}
\label{table:time_by_size}
\end{table}

The baseline times scale as $0.64 \to 3.60 \to 7.25$ ms for $|V|=1{,}000 \to 5{,}000 \to 10{,}000$, closely matching the $O(|V|\log|V|)$ prediction for sparse graphs ($|E|=O(|V|)$). The pruned algorithm's nearly constant time confirms that $|E'|$ grows much slower than $|E|$ as the graph scales.
...
\bottomrule
\end{tabular}
\caption[Space complexity breakdown]{Space complexity breakdown. Identical for both algorithms.}
...
\midrule
1,000  & 65.9 & 5.2 & 12.7$\times$ \\
5,000  & 329.7 & 5.5 & 59.9$\times$ \\
10,000 & 676.4 & 5.5 & 123.0$\times$ \\
\bottomrule
\end{tabular}
\caption[Peak memory scaling by graph size]{Peak memory at $\tau=0.01$. Baseline memory scales linearly with $|V|$ (confirming $O(|V|)$ for sparse graphs). Pruned memory is essentially constant due to aggressive pruning.}
\label{table:memory_by_size}
\end{table}

Baseline peak memory (65.9 KB $\to$ 329.7 KB $\to$ 676.4 KB) scales linearly with $|V|$, confirming the $O(|V|+|E|)$ bound. The pruned algorithm's constant $\approx5.5$~KB at all sizes demonstrates that only a small frontier is ever materialized.
...
\bottomrule
\end{tabular}
\caption[Complexity summary]{Complexity summary. Pruning reduces expected time without affecting worst-case or space bounds.}
\label{table:complexity_summary}
\end{table}

\noindent\textbf{Key empirical findings:}
Baseline wall-clock time scales as $O(|V|\log|V|)$ for sparse graphs, matching theory. Pruned wall-clock time is nearly constant across graph sizes at fixed $\tau$, confirming that $|E'| \ll |E|$. Peak memory scales linearly for the baseline, while pruned memory is effectively constant. Edge-relaxation speedups (up to 2,124$\times$) exceed wall-clock speedups (up to 881$\times$), the difference attributable to fixed Python overhead per function call.
