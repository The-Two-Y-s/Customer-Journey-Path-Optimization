...
The baseline times scale as $1.67 \to 7.12 \to 15.83$ ms for $|V|=1{,}000 \to 5{,}000 \to 10{,}000$, closely matching the $O(|V|\log|V|)$ prediction for sparse graphs ($|E|=O(|V|)$). The pruned algorithm's nearly constant time is consistent with the hypothesis that $|E'|$ grows much slower than $|E|$ as the graph scales (though only three data points are available, so a definitive growth rate cannot be claimed).
...
\bottomrule
\end{tabular}
\caption[Peak memory scaling by graph size]{Peak \texttt{tracemalloc} allocation at $\tau=0.01$, measuring algorithm data structures only (distance dict, parent dict, priority-queue heap); the graph adjacency list is built before tracing begins and is excluded. Baseline memory scales linearly with $|V|$ (confirming $O(|V|)$ for sparse graphs). Pruned memory is essentially constant due to aggressive pruning.}
