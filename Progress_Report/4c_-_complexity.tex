...
\midrule
1,000  & 0.58 & 0.050 & 11.0$\times$ & 30.6$\times$ \\
5,000  & 2.69 & 0.056 & 55.8$\times$ & 104.9$\times$ \\
10,000 & 5.64 & 0.057 & 89.1$\times$ & 173.9$\times$ \\
\bottomrule
\end{tabular}
...
The baseline times scale as $0.58 \to 2.69 \to 5.64$ ms for $|V|=1{,}000 \to 5{,}000 \to 10{,}000$, closely matching the $O(|V|\log|V|)$ prediction for sparse graphs ($|E|=O(|V|)$). The pruned algorithm's nearly constant time confirms that $|E'|$ grows much slower than $|E|$ as the graph scales.
...
\midrule
1,000  & 83.9 & 4.4 & 19.1$\times$ \\
5,000  & 349.6 & 4.6 & 76.0$\times$ \\
10,000 & 662.3 & 4.5 & 147.2$\times$ \\
\bottomrule
\end{tabular}
...
Baseline peak memory (83.9 KB $\to$ 349.6 KB $\to$ 662.3 KB) scales linearly with $|V|$, confirming the $O(|V|+|E|)$ bound. The pruned algorithm's constant $\approx4.5$~KB at all sizes demonstrates that only a small frontier is ever materialized.
