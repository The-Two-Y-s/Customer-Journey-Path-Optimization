...
Pruning reduces the number of edges examined, yielding speedups from $8.8\times$ at $\tau=0.001$ to $2{,}124\times$ at $\tau=0.5$. Whenever the pruned algorithm finds a path, it returns the exact same optimal path as the baseline, with an optimality gap of $0.0000\%$ across all 171 path-found rows. The only cost is reduced reachability: at aggressive $\tau$ values the optimal path may be pruned entirely, causing the algorithm to report no path.
