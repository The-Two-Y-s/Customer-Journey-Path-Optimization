...
Maximising path probability is equivalent to optimising for the most likely conversion sequence. In an e-commerce context, the highest-probability path from a landing page to checkout represents the conversion funnel that customers naturally follow. Identifying this path allows businesses to reduce friction along the dominant route, prioritise UI investments on the states that carry the most traffic, and design nudges that align with observed user behaviour rather than assumed behaviour. The term ``optimisation'' in the project title therefore refers not only to algorithmic speedup but also to the actionable business insight that optimal-path discovery enables.
...
In addition to correctness, the project formally analyzes the theoretical time and space complexity of both the baseline and optimized algorithms. Empirical performance evaluations have been conducted on graphs of varying sizes and densities to examine scalability, runtime behaviour, and trade-offs between computational efficiency and path optimality. Through these analyses, the project demonstrates how careful algorithm design and data structure selection influence performance in large-scale navigation graphs.
...
A further contribution is the identification of the \emph{critical pruning threshold} $\tau_c$, defined as the value or narrow range of $\tau$ for which runtime improvement is maximized without causing any degradation in path quality. Empirical results confirm that the pruning mechanism is ``all-or-nothing''---it either preserves the exact optimal path (0\% gap) or prunes it entirely.

\section{Hypothesis and Verdict}
...
\noindent\textbf{Experimental Verdict: Hypothesis Supported.}

Pruning reduces the number of edges examined, yielding wall-clock speedups from $3.3\times$ at $\tau=0.001$ to $738\times$ at $\tau=0.5$ (edge-relaxation speedups of $8.7\times$ to $1{,}986\times$; fair edge-examination speedups of $1.9\times$ to $711\times$). Whenever the pruned algorithm finds a path, it returns the exact same optimal path as the baseline, with an optimality gap of $0.0000\%$ across all 178 path-found rows (of 1,800 pruned). The only cost is reduced reachability: at aggressive $\tau$ values the optimal path may be pruned entirely, causing the algorithm to report no path.
