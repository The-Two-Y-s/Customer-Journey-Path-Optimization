...
\toprule
\textbf{Algorithm} & \textbf{Counter} & \textbf{Counts} & \textbf{Placement in Code} \\
\midrule
...
Pruned & \texttt{edges\_examined} & Every outgoing edge from a settled node & Before the \texttt{if new\_cost > T} check \\
Pruned & \texttt{edges\_relaxed} & Only edges that \textbf{survived} $\tau$ & After the \texttt{if new\_cost > T: continue} check \\
\bottomrule
\end{tabular}
\caption[Edge-counting semantics]{Edge-counting semantics. \texttt{edges\_examined} provides an apples-to-apples comparison between baseline and pruned, since both algorithms increment it at the same point for every outgoing edge. \texttt{edges\_relaxed} in the pruned algorithm counts only threshold-surviving edges; the difference \texttt{edges\_examined $-$ edges\_relaxed} isolates the pruning overhead.}
