\chapter{ALGORITHM DESIGN, DATA STRUCTURES, AND IMPLEMENTATION}\label{chap:methodology}
...
Figure~\ref{fig:system_workflow} presents the end-to-end workflow. It is organized into five sequential components: (1) data generation and preprocessing, (2) graph construction, (3) baseline path optimization using Dijkstra's algorithm, (4) optimized path search using probability-pruned Dijkstra, and (5) experimental evaluation and comparative analysis.

Though baseline and pruned variants were initially compared, the final implementation adopts a parsimonious unified search engine. Algorithm~1 (Baseline mode) applies standard Dijkstra's algorithm to the log-probability-transformed graph ($O(|E|\log|V|)$), while Algorithm~2 (Pruned mode) introduces a pruning threshold $\tau$ that discards partial paths whose cumulative probability falls below $\tau$ ($O(|E'|\log|V|)$ with $|E'| \leq |E|$). Both behaviors are handled by a single function, `dijkstra_search`, ensuring identical heap-management logic across variants.
...
\end{tikzpicture}
\caption[System workflow overview]{End-to-end system workflow.}
...
Figure~\ref{fig:journey_graph} illustrates a representative five-node customer journey graph. Node abbreviations: LP\,=\,Landing Page (source~$s$), SR\,=\,Search Results, PP\,=\,Product Page, CT\,=\,Cart, CK\,=\,Checkout (target~$t$). The solid blue path LP\,$\to$\,SR\,$\to$\,PP\,$\to$\,CK is optimal ($\Pi^*=0.336$, log-cost $\approx 1.091$).
...
\end{tikzpicture}
\caption[Example customer journey graph]{Five-node customer journey graph ($s=\text{LP}$, $t=\text{CK}$).}
...
For a given source--target query, the system returns the optimal path $P^* = (v_0 = s, v_1, \ldots, v_k = t)$, the corresponding path probability $\Pi^* = \prod_{(u,v)\in P^*} p(u,v) = \exp(-C^*)$, and the log-cost $C^* = \sum_{(u,v)\in P^*} -\log\, p(u,v)$. Each run also records performance metrics---execution time, peak memory, nodes explored, edges examined, edges relaxed, and maximum priority queue size---as well as the optimality gap $\Delta = |C^*_{\text{baseline}} - C^*_{\text{pruned}}| / C^*_{\text{baseline}} \times 100\%$.

\paragraph{Concrete worked example: RetailRocket event-level funnel.}
The RetailRocket dataset~\citep{retailrocket2015} yields a compact 3-node graph.
Its MLE transition probabilities, computed from 2.7\,million events, provide a concrete illustration of the formulation:
...
\midrule
view $\to$ addtocart    & $p = 0.041$ & $w = -\ln(0.041) = 3.19$ \\
addtocart $\to$ transaction & $p = 0.226$ & $w = -\ln(0.226) = 1.49$ \\
view $\to$ transaction (direct) & $p = 0.004$ & $w = -\ln(0.004) = 5.52$ \\
\bottomrule
\end{tabular}
\end{center}
...
def generate_erdos_renyi_graph(n, avg_degree, distribution, seed, ...):
    ...
    for u in graph:
        ...
        for v in targets:
            ...
            else:  # power_law: Pareto(alpha=2): x_min * (1-U)^(-1/alpha) with alpha=2
                raw = min(0.01 * (1 - rng.random()) ** (-0.5), 1.0)
            graph[u][v] = raw
...
\caption[Real-world dataset summary]{Real-world clickstream datasets used for validation.}
...
\KwOut{Shortest-path distance $\mathit{best\_costs}[t]$, probability $\exp(-\mathit{best\_costs}[t])$, path $P^*$}
Initialise $\mathit{best\_costs}[v] \leftarrow \infty$ for all $v \in V$; $\mathit{parent}[v] \leftarrow \mathrm{NIL}$\;
$\mathit{best\_costs}[s] \leftarrow 0$; insert $(0, s)$ into min-heap $Q$\;
\While{$Q$ is not empty}{
  $(d, u) \leftarrow \mathrm{Extract\text{-}Min}(Q)$\;
  \If{$u = t$}{\textbf{break}\tcp*{optimal path reached}}
  \If{$d > \mathit{best\_costs}[u]$}{\textbf{continue}\tcp*{stale entry (lazy deletion)}}
  \ForEach{edge $(u, v)$ with weight $w = -\log p(u,v)$}{
    $\mathit{edges\_examined} \leftarrow \mathit{edges\_examined} + 1$\;
    $\mathit{edges\_relaxed} \leftarrow \mathit{edges\_relaxed} + 1$\tcp*{no pruning, so every examined edge is relaxed}
    $\mathit{new\_dist} \leftarrow \mathit{best\_costs}[u] + w$\;
    \If{$\mathit{new\_dist} < \mathit{best\_costs}[v]$}{
      $\mathit{best\_costs}[v] \leftarrow \mathit{new\_dist}$; $\mathit{parent}[v] \leftarrow u$\;
      Insert $(\mathit{new\_dist}, v)$ into $Q$\;
    }
  }
}
\Return $\mathit{best\_costs}[t]$, $\exp(-\mathit{best\_costs}[t])$, $\mathrm{ReconstructPath}(\mathit{parent}, s, t)$\;
\end{algorithm}
...
\bottomrule
\end{tabular}
\caption[Dijkstra best\_costs trace]{Dijkstra \texttt{best\_costs[]} trace ($s=\text{LP}$, $t=\text{CK}$).}
\label{table:dijkstra_trace}
\end{table}

The column headers are node abbreviations: LP\,=\,Landing Page (source~$s$), SR\,=\,Search Results, PP\,=\,Product Page, CT\,=\,Cart, and CK\,=\,Checkout (target~$t$). Each row shows the \texttt{best\_costs[]} array after extracting the minimum-cost node listed in the \textbf{Extract} column. Values are log-costs $w=-\log\,p$; $\infty$ denotes an unreached node. At Step~1, extracting LP relaxes its neighbours SR ($-\log0.7\approx0.36$) and CT ($-\log0.3\approx1.20$). At Step~2, SR is extracted and reaches PP ($0.36+{-\log0.6}\approx0.87$). Step~3 extracts PP and discovers CK ($0.87+{-\log0.8}\approx1.09$). At Step~4, CK is extracted with cost $\mathbf{1.09}$; the \checkmark{} annotation marks the \emph{target-reached} early exit. The bold value $\mathbf{1.09}$ is the final optimal log-cost, corresponding to path probability $\Pi^*=\exp(-1.09)\approx0.336$.
...
\While{$Q$ is not empty}{
  $(d, u) \leftarrow \mathrm{Extract\text{-}Min}(Q)$\;
  \If{$u = t$}{\textbf{break}}
  \If{$d > \mathit{best\_costs}[u]$}{\textbf{continue}\tcp*{stale entry}}
  \ForEach{edge $(u, v)$ with weight $w = -\log p(u,v)$}{
    $\mathit{edges\_examined} \leftarrow \mathit{edges\_examined} + 1$\;
    $\mathit{new\_dist} \leftarrow \mathit{best\_costs}[u] + w$\;
...
\toprule
\textbf{Algorithm} & \textbf{Metric} & \textbf{Definition} & \textbf{Code Placement} \\
\midrule
Baseline & \texttt{edges\_examined} & Every outgoing edge from a settled node & Before the improvement check \\
Baseline & \texttt{edges\_relaxed} & Same as \texttt{edges\_examined} & No pruning step \\
Pruned & \texttt{edges\_examined} & Every outgoing edge from a settled node & Before \texttt{if new\_cost > T} \\
Pruned & \texttt{edges\_relaxed} & Only edges that \textbf{survived} $\tau$ & After \texttt{if new\_cost > T: continue} \\
\bottomrule
\end{tabular}%
}
\caption[Edge-counting semantics]{Edge-counting semantics for baseline and pruned Dijkstra.}
\label{table:edges_relaxed}
\end{table}

Examination speedup is defined as the baseline-to-pruned ratio for \texttt{edges\_examined}, providing a fair like-for-like comparison.
Relaxation speedup is defined as the baseline-to-pruned ratio for \texttt{edges\_relaxed}; it captures pruning aggressiveness, but is not a direct efficiency comparison because the two denominators count different populations.

\subsubsection{Actual Python Implementation}

The following is the actual pruned Dijkstra implementation from the codebase:

\begin{lstlisting}[language=Python, caption={Unified Dijkstra search engine (\texttt{src/dijkstra.py})}, label=lst:unified_impl]
def dijkstra_search(graph: Graph, start: str, goal: str,
                   tau: float = 0.0) -> DijkstraResult:
    ...
    while priority_queue:
        ...
        for neighbor, weight in graph.get(node, []):
            ...
            if new_cost > T:
                continue  # PRUNE: path probability below tau
...
\begin{tikzpicture}[
    ...
    terminal/.style={rectangle, rounded corners=8pt, minimum width=2.2cm, minimum height=0.6cm,
                     draw=nodegreen!80!black, thick, fill=boxgreen, font=\small\bfseries, text centered},
...
\node[terminal, right=1.0cm of b_tgt]    (b_done)   {Return path};

\draw[arr] (b_start) -- (b_tgt);
\draw[arr] (b_tgt)   -- node[lbl,left]{No}  (b_stale);
\draw[arr] (b_stale) -- node[lbl,right]{No} (b_relax);
\draw[arr] (b_relax) --                     (b_push);

% u=t? Yes → Return path (terminal exit)
\draw[arr] (b_tgt)   -- node[lbl,above]{Yes} (b_done);

% Stale? Yes → loop back to Extract-Min
\draw[arr] (b_stale.west) -- ++(-0.7,0)
    node[lbl, left]{Yes}
    |- (b_start.west);

% Push → loop back to Extract-Min
\draw[arr] (b_push.west) -- ++(-1.2,0) |- (b_start.west);

\node[font=\small\bfseries, above=0.25cm of b_start, color=nodeorange] {Baseline Dijkstra};

%--- RIGHT COLUMN: Pruned Dijkstra ---
\node[startstop, fill=boxorange, right=6.5cm of b_start] (p_start) {Extract-Min$(Q)$};
\node[decision,  below=0.5cm of p_start]  (p_tgt)   {$u = t$?};
\node[decision,  below=0.7cm of p_tgt]    (p_stale)  {Stale?};
\node[decision,  below=0.7cm of p_stale]  (p_foreach) {More edges?};
\node[decision,  below=0.7cm of p_foreach] (p_prune)  {$d_{\text{new}} > T$?};
\node[prune,     right=1.5cm of p_prune]  (p_skip)   {\textbf{PRUNE} (skip)};
\node[process,   below=0.7cm of p_prune, fill=boxblue] (p_relax)  {Relax \& Push if better};
\node[terminal,  right=1.0cm of p_tgt]    (p_done)   {Return path};

\draw[arr] (p_start) -- (p_tgt);
\draw[arr] (p_tgt)   -- node[lbl,left]{No}  (p_stale);
\draw[arr] (p_stale) -- node[lbl,right]{No} (p_foreach);
\draw[arr] (p_foreach) -- node[lbl,right]{Yes} (p_prune);
\draw[arr] (p_prune) -- node[lbl,right]{No} (p_relax);

% u=t? Yes → Return path (terminal exit)
\draw[arr] (p_tgt)   -- node[lbl,above]{Yes} (p_done);

% Stale? Yes → loop back to Extract-Min
\draw[arr] (p_stale.west) -- ++(-0.7,0)
    node[lbl, left]{Yes}
    |- (p_start.west);

% More edges? No → all edges processed, back to Extract-Min
\draw[arr] (p_foreach.west) -- ++(-1.2,0)
    node[lbl, left]{No}
    |- (p_start.west);

% PRUNE: Yes → skip edge, back to More edges?
\draw[arr] (p_prune) -- node[lbl,above]{Yes} (p_skip);
\draw[arr] (p_skip.north) |- (p_foreach.east);

% Relax → loop back to More edges? (next edge in inner loop)
\draw[arr] (p_relax.east) -- (p_relax.east -| p_skip.east) -- ++(0.5,0) |- (p_foreach.east);

\node[font=\small\bfseries, above=0.25cm of p_start, color=nodegreen] {Pruned Dijkstra};
\end{tikzpicture}%
}% end resizebox
\caption[Algorithm decision-flow comparison]{Decision-flow comparison of baseline and pruned Dijkstra.}
...
\midrule
\textbf{Optimality} & Guaranteed globally optimal & Optimality is guaranteed if and only if the optimal path satisfies $C^* \leq T$; otherwise, the algorithm reports infeasibility \\[4pt]
...
\textbf{Convergence} & Always optimal & Identical to baseline as $\tau \to 0$; as $\tau$ decreases, the feasible search space expands monotonically \\
\bottomrule
\end{tabular}%
}
\caption[Algorithm comparison]{Comparison of baseline and pruned Dijkstra.}
\label{table:algo_compare}
\end{table}

\subsection{Critical-\texorpdfstring{$\tau$}{tau} Finder}
...
\subsection{\texorpdfstring{$k$}{k}-Shortest Simple Paths}

In addition to single-path queries, the CLI (\texttt{main.py}) provides a $k$-shortest simple paths mode activated by the \texttt{--k} flag. This uses a best-first search with visited-set tracking to enumerate the top-$k$ distinct simple paths from $s$ to $t$ in non-decreasing cost order. When $k=1$, the result is identical to Dijkstra's shortest path. This feature is validated by five dedicated unit tests covering $k=1$ equivalence, ascending cost ordering, simple-path guarantee, $k=0$ boundary, and disconnected-graph behavior.

\textbf{Complexity note:} Enumerating $k$ simple paths via best-first search has worst-case exponential complexity $O(k \cdot |V|!)$, since the number of simple paths in a dense graph can be factorial in $|V|$. In practice, the search terminates early once $k$ paths are found, and the \texttt{max\_path\_len} parameter bounds the depth of exploration. This utility is provided for interactive analysis of small queries and is not part of the theoretically analyzed pruning framework.
...
Python's \texttt{heapq} module provides $O(\log|V|)$ extract-min and insert. Since \texttt{heapq} does not support decrease-key, \textbf{lazy insertion} is used: when a shorter path to a node is found, a new entry is pushed to the \texttt{priority\_queue}; stale entries are skipped when popped (the \texttt{if cost > best\_costs.get(node, ...)} check).

\subsection{Hash Maps (Dictionaries)}

\texttt{best\_costs[v]} and \texttt{parent[v]} are Python dictionaries with $O(1)$ average-case access.
...
Table~\ref{table:simenv} summarizes the environment used for all experiments.
...
\midrule
Programming Language & Python 3.12.3 (standard library only for core algorithms) \\
Graph Representation & Adjacency list (\texttt{dict} of \texttt{list}) \\
...
Execution time and peak memory are measured via \texttt{time.perf\_counter()} and \texttt{tracemalloc.get\_traced\_memory()}, respectively.
Each run also records nodes explored (vertices extracted from the priority queue), edges examined (total outgoing edges visited from settled nodes), edges relaxed (edge operations that survived the $\tau$ threshold; see Table~\ref{table:edges_relaxed}), and maximum priority queue size.
The path probability is $\Pi^* = \exp(-C^*)$. The optimality gap is defined as:
    \[
...
\bottomrule
\end{tabular}
\caption[Experimental parameter matrix]{Experimental parameter matrix ($2{,}160$ total rows).}
