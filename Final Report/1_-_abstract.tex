...
Evaluation spans 3,000 configurations---2,160 synthetic (graphs up to 10,000 nodes) and 840 real-data rows from RetailRocket and RecSys~2015---under fixed and adaptive $\tau$ regimes. Across all 706 path-found rows the optimality gap is $0.0000\%$, consistent with the all-or-nothing property (Theorem~4.5). When the pruned algorithm successfully recovers a path, median wall-clock speedups range from $1.6\times$ to $7.2\times$ on synthetic data and up to $5.8\times$ on real-world graphs. When the threshold prunes the optimal path entirely, the algorithm detects infeasibility orders of magnitude faster than the baseline (median up to $738\times$ on synthetic data and $18{,}882\times$ on real-world item-level graphs). On large sparse real-data graphs, an adaptive-$\tau$ regime raises path-found rates from near~0\% to over~80\%, demonstrating that threshold-based pruning provides both fast infeasibility detection and correct path recovery with guaranteed optimality when a path is returned.
