...
\noindent\textbf{Contribution.} This study presents a \textbf{controlled pruning framework} for probabilistic shortest-path computation, grounded in classical algorithm design and supported by comprehensive empirical evaluation. The pruning variant is deterministic, requires no domain-specific heuristic (unlike A*~\citep{hart1968astar}), and provably converges to the globally optimal solution as $\tau \to 0$ (unlike rank-based methods such as beam search, which provide no optimality guarantee~\citep{lowerre1976speech,jurafsky2000speech}). The principal contributions are:
\begin{enumerate}
    \item A formal characterization of an \emph{all-or-nothing} correctness property (Theorem~\ref{thm:all_or_nothing}): the algorithm returns either the exact optimal path or reports no path found, with no sub-optimal intermediate results.
    \item Identification and empirical characterization of the \emph{critical pruning threshold}~$\tau_c$---a narrow operational range in which speedup is maximized while preserving path optimality.
    \item A systematic evaluation framework spanning synthetic and real-world datasets, demonstrating scalability to graphs with tens of thousands of nodes while maintaining solution quality.
\end{enumerate}

\medskip
\noindent\textbf{Novelty and Prior Work.} While pruning strategies, heuristic search, and threshold-based filtering are individually well-studied in shortest-path literature~\citep{hart1968astar,pearl1984heuristics,cormen2009}, to the best of our knowledge, such integration has not been extensively studied in the context of probabilistic customer journey graphs. Specifically, the combination of (i)~a deterministic, parameter-controlled pruning mechanism with provable convergence, (ii)~a formal all-or-nothing optimality guarantee (Theorem~\ref{thm:all_or_nothing}), and (iii)~an adaptive-$\tau$ regime empirically validated on real-world e-commerce graphs has not been previously reported. This positions the present work at the intersection of classical graph algorithms and applied clickstream optimization, addressing a gap identified in Chapter~3.

Formally, $\tau$ acts as a lower bound on acceptable cumulative path probability, controlling the trade-off between search completeness and computational efficiency.
