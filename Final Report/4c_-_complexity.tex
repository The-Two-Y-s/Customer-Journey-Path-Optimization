...
\begin{enumerate}
    \item \textbf{Initialization:} Setting $\mathit{best\_costs}[v]=\infty$ and $\mathit{parent}[v]=\texttt{NIL}$ for all $v$ costs $O(|V|)$.
...
\begin{enumerate}
    ...
    \item \textbf{Empirically observed case ($\tau>0$):} Let $|E'|\leq|E|$ denote edges surviving the threshold. Pruned edges are checked in $O(1)$ using hash maps for $O(1)$ cost lookups, ensuring scalability. The observed complexity is:
    \[
...
Cost map $\mathit{best\_costs}[v]$ & $O(|V|)$ & Shortest path estimates \\
