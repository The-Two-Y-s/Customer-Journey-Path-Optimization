...
Upon termination, $\mathit{best\_costs}[t]$ equals the true shortest-path distance $\delta(s,t)$ in the log-transformed graph.
\end{theorem}

\begin{proof}
We rely on the standard inductive proof for Dijkstra’s algorithm~\citep{cormen2009}. The algorithm maintains the invariant that for every node $u$ extracted from the priority queue, $\mathit{best\_costs}[u] = \delta(s,u)$. Because all edge weights $w(u,v) \geq 0$, the extraction of a node $u$ with the minimum tentative distance guarantees that no shorter path can exist through nodes currently in the queue or yet to be discovered. 
\end{proof}
...
\begin{proof}
As $\tau \to 0^+$, we have $T = -\log(\tau) \to +\infty$. In the unified `dijkstra_search` implementation, setting $\tau = 0.0$ is equivalent to $T = \mathtt{math.inf}$. The pruning condition $\mathit{new\_cost} > T$ is never satisfied since all finite path costs are strictly less than infinity. Consequently, no edge is pruned, and the execution flow becomes identical to the baseline Dijkstra algorithm. Correctness follows directly from Theorem~\ref{thm:baseline_optimal}.
\end{proof}
