...
class SyntheticJourneyGenerator:
    ...
    def generate(self, num_sessions: int = 1000) -> pd.DataFrame:
        """
        ...
        for session_id in range(num_sessions):
            ...
            # Session-start time
            state_time = base_time + timedelta(minutes=np.random.randint(0, 43200)) # Within a month
            
            max_steps = max(2, np.random.poisson(self.avg_session_length))
            ...
            while current_state not in ['Checkout', 'Exit'] and step <= max_steps:
                ...
                next_state = np.random.choice(targets, p=probs)
                ...
                step += 1
                state_time += timedelta(seconds=np.random.randint(5, 300)) # Time between clicks
